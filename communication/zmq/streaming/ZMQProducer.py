            ...
            for type, workerQueu in workers.queues.items():
                if (workerQueu and self.messageQueue.hasItems(type)):
                    if (len(self.messageQueue.queue) % 100 == 0):
                        print('In queue for ' + type + ' ' + str(len(self.messageQueue.queue)))
...
if __name__ == "__main__":
    ...
    def doNothing(msg, type):
        responses[0] += 1


    producer = ZMQProducer(port=5556, maxMemorySize=5000, responseAcumulator=doNothing, consumerTypes=['a'])
    gevent.spawn(producer.start)
    for i in itertools.count():
        producer.produce(b'bb1' + bytes(i))
        gevent.sleep(0.01)
        sent[0] += 1
        if (sent[0] % 1000 == 0):
            print('Sent count:' + str(sent[0]) + '\nResponses count:' + str(responses[0]) + '\nQueue count:' + str(
                len(producer.messageQueue.queue)))
